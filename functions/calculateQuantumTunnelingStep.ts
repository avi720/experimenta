...
/**
 * @typedef {Object} QuantumTunnelingParameters
 * @property {number} v - Particle speed as fraction of c
 * @property {number} m - Particle mass as fraction of electron mass  
 * @property {number} V0 - Barrier height in eV
 * @property {number} L - Barrier width in nm
 * @property {number} dt - Simulation time step / sampling rate
 * @property {boolean} measurement_mode - Measurement detector on/off
 */

/**
 * @typedef {Object} QuantumTunnelingDataPoint
...
/**
 * @typedef {Object} TransmissionReflectionResult
 * @property {number} transmission
 * @property {number} reflection
 */
 ...
 * @param {number} E_si - Energy in SI units
 * @param {number} V0_si - Barrier height in SI units
 * @param {number} L_si - Barrier width in SI units
 * @param {number} m_si - Mass in SI units
 * @returns {TransmissionReflectionResult}
 */
...
/**
 * Calculate quantum tunneling step
 * @param {number} t - Time
 * @param {QuantumTunnelingParameters} parameters - Simulation parameters
 * @returns {QuantumTunnelingDataPoint}
 */
function calculateStep(t, parameters) {
    ...
    const v_num = parseFloat(v.toString()) || 0.01;
    const m_num_ui_unit = parseFloat(m.toString()) || 5.11;
    const V0_num = parseFloat(V0.toString()) || 1.0;
    const L_num = parseFloat(L.toString()) || 0.005;
...
Deno.serve(async (req) => {
    try {
        // The base44 client and user authentication check are removed to optimize for performance,
        // as this endpoint does not require user context.
        ...
        console.error('Quantum tunneling calculation error:', error);
